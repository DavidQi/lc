...
def frog_jump(stones, s=0):
    ...
    for i, u in enumerate(stones):
        ...
        if u == stones[-1]:
            return True
        for c in [u+s-1, u+s, u+s+1]:
            if c in stones[i+1:]:
                return frog_jump(stones[stones.index(c):], s)
        return False

if __name__ == '__main__':
    print(frog_jump([0, 1, 3, 5, 6, 8, 12, 17]))
    print(frog_jump([0, 1, 2, 3, 4, 8, 9, 11]))
